...
/**
 * 默认的任务处理方式2----将所有的命令缓存起来。等到音频路径输入的时候，才开始进行任务的分发
 * 在这里处理业务逻辑。
 * 对任务进行分发。
 * <p>
 * 处理任务的逻辑和1的逻辑相同个，不变。如下：
 ...
 * <p>
 * 简化的逻辑是（等级就是level）：
 * 1.任务完成之后去找下一个任务。再分发q出去。
 * 如果完成的是 低级任务。下一个任务是高级任务。则会将低级任务进行汇总和升级。升级过后才进行下一个高级任务。
 *
 * @author zzx
 * @date 2017/5/8.
 */

public class CmdDistributionImpl2 implements CmdDistribution, AbsTask.TaskCompletedCallBack {
     ...
     */
    Gson gson = new Gson();
    Type type = new TypeToken<LinkedList<CommandType>>() {
    }.getType();
    ...
    //cmd的集合。
    private LinkedList<CommandType> commandTypes;


    public CmdDistributionImpl2(Context context) {
    ...
    //从缓存中取
    private LinkedList<CommandType> getFromSharePref(String course_id) {
        HashMap<String, String> command_list = SharedPreferencesUtils.getObject(FFmpegKit.getContext(),"Command_list", HashMap.class);
        if (command_list == null) {
            command_list = new HashMap<>();
        }
        String s = command_list.get(course_id);
        LinkedList<CommandType> getFromList;
        try {
            getFromList = gson.fromJson(s, type);
        } catch (Exception e) {
            //解析失败。则说明并不存在。
            getFromList = new LinkedList<>();
            //再设置回去。
    ...
    private void findNextComandType() {
        //取出第一个。开始执行命令。
        CommandType commandType = commandTypes.pollLast();
...
//        errorList.add(task);
     ...
     * 正在触发开始处理可见命令的方法
     *
     * @param finalInputAudioPath
     * @return
     */
    public CmdDistributionImpl2 setFinalInputAudioPath(String finalInputAudioPath) {
...
//        doWhenCourseActSetFAudio(finalInputAudioPath);
    ...
    public CmdDistributionImpl2 setProgressCallBack(ProgressCallBack progressCallBack) {
    ...
    public CmdDistributionImpl2 setScaleListener(OnScaleVideoListener scaleListener) {
